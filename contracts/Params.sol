...
contract Params {
    ...
    function set_params(
        string memory _name,
        // G.G1Point[] memory _hs,
        G.G2Point memory _alpha,
        G.G1Point[] memory _g1_beta,
        G.G2Point[] memory _beta,
        string[][] memory _combinations,
        string[] memory _dependent_ttps,
        uint256[][] memory _include_indexes,
        uint256[] memory _public_m_encoding,
        G.G1Point[] memory _hs
    ) public onlyOwner {
    ...
    function set_ttp_params(
        string memory _name,
        G.G1Point memory _ttp_pk,
        G.G1Point[] memory _hs
    ) public onlyOwner {
    ...
    function get_public_m_encoding(
        string memory _name
    ...
    function get_alpha(
        string memory _name
    ) public view returns (G.G2Point memory) {
        uint256 id = getMapCredentials(_name);
        require(id != 0, "No such AC.");
        return alpha[id];
    }
        ...
        for (uint256 i = 0; i < all_combinations.length; i++) {
            if (all_combinations[i].length == combination.length) {
                ...
                for (j = 0; j < combination.length; j++) {
                    if (
                        keccak256(bytes(all_combinations[i][j])) !=
                        keccak256(bytes(combination[j]))
                    ) {
                        break;
                    }
                }
    ...
    function get_include_indexes(
        ...
        require(id != 0, "No such AC.");
        require(
            checkCombination(id, combination),
            "Such a combination is not possible."
        );
        uint256[][] memory _include_indexes = new uint256[][](
            combination.length
        );
        for (uint256 i = 0; i < combination.length; i++) {
            _include_indexes[i] = new uint256[](
                include_indexes[id][combination[i]].length
            );
